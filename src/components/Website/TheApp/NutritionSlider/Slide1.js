...
const Slide1 = forwardRef((props, ref) => {
  const query = graphql`
    query {
      steakMobile: file(
  ...
  const mobile = images.steakMobile;
  const tablet = images.steakTablet;
  const laptop = images.steakLaptop;
    ...
    <NutritionGrid ref={ref}>
      <NutritionBackground>
        {backgroundReady ? (
          <Image
            fluid={background}
            alt="Steak salad with avacodo sauce"
            title="Steak salad with avacodo sauce"
          />
        ) : (
